"use client"
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { signInSchema } from "../../../../formSchema/SignInUpSchema";
import Link from "next/link";

export default function SignInSignUpPage() {
  const {
    register: registerSignIn,
    handleSubmit: handleSignIn,
    formState: { errors: signInErrors },
  } = useForm({
    resolver: zodResolver(signInSchema),
  });


  const onSubmitSignIn = (data) => {
    console.log("Sign In Data: ", data);
  };


  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-indigo-700 p-6">
      {/* Sign In Form */}
      <div className="bg-white p-6 rounded-md shadow-md w-full max-w-md">
        <h2 className="text-2xl font-semibold text-black mb-6 text-center">Sign In</h2>
        <form onSubmit={handleSignIn(onSubmitSignIn)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Email</label>
            <input
              type="email"
              {...registerSignIn("email")}
              className="w-full p-2 mt-1 border rounded-md"
            />
            {signInErrors.email && (
              <p className="text-red-500 text-sm">{signInErrors.email.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700">Password</label>
            <input
              type="password"
              {...registerSignIn("password")}
              className="w-full p-2 mt-1 border rounded-md"
            />
            {signInErrors.password && (
              <p className="text-red-500 text-sm">{signInErrors.password.message}</p>
            )}
          </div>

          <button
            type="submit"
            className="w-full bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-800"
          >
            Sign In
          </button>
        </form>

        {/* Sign In with Google */}
        <div className="mt-4 text-center">
          <button className="w-full bg-white flex items-center justify-center text-black border border-black py-2 rounded-lg hover:bg-red-600">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="20px" height="20px"><path d="M 26 2 C 13.308594 2 3 12.308594 3 25 C 3 37.691406 13.308594 48 26 48 C 35.917969 48 41.972656 43.4375 45.125 37.78125 C 48.277344 32.125 48.675781 25.480469 47.71875 20.9375 L 47.53125 20.15625 L 46.75 20.15625 L 26 20.125 L 25 20.125 L 25 30.53125 L 36.4375 30.53125 C 34.710938 34.53125 31.195313 37.28125 26 37.28125 C 19.210938 37.28125 13.71875 31.789063 13.71875 25 C 13.71875 18.210938 19.210938 12.71875 26 12.71875 C 29.050781 12.71875 31.820313 13.847656 33.96875 15.6875 L 34.6875 16.28125 L 41.53125 9.4375 L 42.25 8.6875 L 41.5 8 C 37.414063 4.277344 31.960938 2 26 2 Z M 26 4 C 31.074219 4 35.652344 5.855469 39.28125 8.84375 L 34.46875 13.65625 C 32.089844 11.878906 29.199219 10.71875 26 10.71875 C 18.128906 10.71875 11.71875 17.128906 11.71875 25 C 11.71875 32.871094 18.128906 39.28125 26 39.28125 C 32.550781 39.28125 37.261719 35.265625 38.9375 29.8125 L 39.34375 28.53125 L 27 28.53125 L 27 22.125 L 45.84375 22.15625 C 46.507813 26.191406 46.066406 31.984375 43.375 36.8125 C 40.515625 41.9375 35.320313 46 26 46 C 14.386719 46 5 36.609375 5 25 C 5 13.390625 14.386719 4 26 4 Z" /></svg>
            <span className="ml-2">Google</span>
          </button>
        </div>

        {/* Option to Sign Up */}
        <p className="mt-4 text-center text-slate-500">
          Don't have an account?{" "}
          <Link href="/Signup" className="text-indigo-600 hover:underline">
            Sign Up
          </Link>
        </p>
      </div>

    </div>
  );
}
